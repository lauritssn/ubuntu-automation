...
replace_script_variables() {
    ...
    # Replace email configuration - construct full email address
    if [ -n "$INFO_EMAIL" ] && [ -n "$EMAIL_DOMAIN" ]; then
        # If INFO_EMAIL contains @, use it as-is, otherwise append EMAIL_DOMAIN
        if [[ "$INFO_EMAIL" == *"@"* ]]; then
            FULL_EMAIL="$INFO_EMAIL"
        else
            FULL_EMAIL="${INFO_EMAIL}@${EMAIL_DOMAIN}"
        fi

        # Replace both template patterns
        sed -i 's|{{INFO_EMAIL}}|'${FULL_EMAIL}'|g' "$script_path"
        sed -i 's|INFO_EMAIL@EMAIL_DOMAIN|'${FULL_EMAIL}'|g' "$script_path"
    elif [ -n "$INFO_EMAIL" ]; then
        # Use INFO_EMAIL as-is if EMAIL_DOMAIN is not set
        sed -i 's|{{INFO_EMAIL}}|'${INFO_EMAIL}'|g' "$script_path"
        sed -i 's|INFO_EMAIL@EMAIL_DOMAIN|'${INFO_EMAIL}'|g' "$script_path"
    fi

    # Replace individual components for backward compatibility
    if [ -n "$INFO_EMAIL" ]; then
        sed -i 's|{{INFO_EMAIL}}|'${INFO_EMAIL}'|g' "$script_path"
        # Handle old pattern without duplication
        sed -i 's|INFO_EMAIL\([^@]\)|'${INFO_EMAIL}'\1|g' "$script_path"
    fi
