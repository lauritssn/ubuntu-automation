...
# Supports both IPv4 and IPv6 socket listeners
...
show_usage() {
    ...
    echo "Automatically creates both IPv4 and IPv6 listeners for each port"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  $0 traefik 80 443 8080"
    echo "  $0 nginx 80"
    echo "  $0 myapp 3000"
    echo ""
    echo "Output:"
    echo "  Creates: ~/.config/systemd/user/<service-name>.socket"
    echo "  Supports: IPv4 and IPv6 listeners for all specified ports"
    echo ""
...
# Add listening addresses for each port (both IPv4 and IPv6)
for port in "${PORTS[@]}"; do
...
echo "- Ports: ${PORTS[*]}"
echo "- IPv4 and IPv6 listeners created for each port"
echo ""
echo "📋 Next steps:"
